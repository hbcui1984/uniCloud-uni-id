...
var userCollectionName = "uni-id-users";
var tokenSecret = "your-pwd-sec";
var tokenExpiresIn = 3600;
...
let uniToken$1 = {
    ...
    checkToken: async function(token) {
        try {
            let payload = jsonwebtoken.verify(token, tokenSecret);
    ...
    if (userInDB && userInDB.data && userInDB.data.length > 0) {
        ...
        if (encryptPwd(user.password) == pwdInDB) {
            try {
                ...
                let token = uniToken$1.createToken(userInDB.data[0]);
...
const checkToken = uniToken$1.checkToken;
